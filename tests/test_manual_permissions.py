...
def test_permissions_manualRepayFromManager(
    levered_strat, gov, user, management, token
):
    chain.sleep(1)  ## Seems like otherwise it loose tracks of funds when re-setting

    token.approve(
        levered_strat, 10, {"from": user}
    )  ## Gov has a little bit of performance fees
    token.approve(
        levered_strat, 10, {"from": gov}
    )  ## Gov has a little bit of performance fees
    token.approve(
        levered_strat, 10, {"from": management}
    )  ## Gov has a little bit of performance fees
...
def test_permissions_manualCooldownRewards_management(
    levered_strat, gov, user, management
):
    chain.sleep(1)  ## Seems like otherwise it loose tracks of funds when re-setting
    levered_strat.manualClaimRewards({"from": management})
    with brownie.reverts():
...
def test_permissions_manualRedeemRewards_management(
    levered_strat, gov, user, management
):
    chain.sleep(1)  ## Seems like otherwise it loose tracks of funds when re-setting
    levered_strat.manualClaimRewards({"from": management})
...
def test_permissions_manualSwapFromStkAAVEToAAVE_user_gov(
    levered_strat, gov, user, management, reward
):
    chain.sleep(1)  ## Seems like otherwise it loose tracks of funds when re-setting
    ...
    with brownie.reverts():
        levered_strat.manualSwapFromStkAAVEToAAVE(
            reward.balanceOf(levered_strat), 10 ** 18 * 0.95, {"from": user}
        )
...
def test_permissions_manualSwapFromStkAAVEToAAVE_user_management(
    levered_strat, gov, user, management, reward
):
    chain.sleep(1)  ## Seems like otherwise it loose tracks of funds when re-setting
    ...
    levered_strat.manualSwapFromStkAAVEToAAVE(
        reward.balanceOf(levered_strat), 10 ** 18 * 0.95, {"from": management}
    )


def test_permissions_manualSwapFromAAVEToWant_gov(
    levered_strat, gov, user, management, reward, aave
):
    levered_strat.manualClaimRewards({"from": gov})
    ...
    with brownie.reverts():
        levered_strat.manualSwapFromAAVEToWant(
            aave.balanceOf(levered_strat), 0.00589100 * 10 ** 8, {"from": user}
        )

    levered_strat.manualSwapFromAAVEToWant(
        aave.balanceOf(levered_strat), 0.00589100 * 10 ** 8, {"from": gov}
    )


def test_permissions_manualSwapFromAAVEToWant_management(
    levered_strat, gov, user, management, reward, aave
):
    levered_strat.manualClaimRewards({"from": gov})
    ...
    levered_strat.manualSwapFromAAVEToWant(
        aave.balanceOf(levered_strat), 0.00589100 * 10 ** 8, {"from": management}
    )
