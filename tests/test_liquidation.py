...
def get_health(lending_pool, strategy):
  totalCollateralETH, totalDebtETH, availableBorrowsETH, currentLiquidationThreshold, ltv, healthFactor = lending_pool.getUserAccountData(strategy)
...
def test_liquidation(
  ...
  lending_pool.liquidationCall(token, token, levered_strat, to_liquidate, False, {"from": gov})
  chain.mine(1)
